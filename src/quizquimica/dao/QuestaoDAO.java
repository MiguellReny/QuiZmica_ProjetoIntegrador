...
public class QuestaoDAO {
    ...
    public boolean inserir(Questao questao) {
        ...
        String sql =
                ...
                "(enunciado, perguntaImagem, dificuldade, dica, tipo) " +
                "VALUES (?, ?, ?, ?, ?)";
        ...
        try (
            ...
            for (Alternativa alt : questao.getAlternativas()) {
                ...
                psAlt.setBoolean(
                        2,
                        alt.isAlternativaCorreta()
                );

                psAlt.setString(
                        3,
                        alt.getAlternativaImagem()
                );
    ...
    private List<Alternativa> buscarAlternativas(
        ...
        try (
            ...
            while (rs.next()) {
                ...
                alternativa.setAlternativa(
                        rs.getString("alternativa")
                );

                alternativa.setAlternativaCorreta(
                        rs.getBoolean(
                                "alternativaCorreta"
                        )
                );

                alternativa.setIdQuestao(
                        rs.getInt(
                                "perguntas_idPerguntas"
                        )
                );

                alternativa.setAlternativaImagem(
                        ConversorImagemUrl.converter(
                                rs.getString(
                                        "alternativaImagem"
                                )
                        )
    ...
    private Questao mapearQuestao(ResultSet rs)
            throws SQLException {
        ...
        questao.setImagemUrl(
                rs.getString("perguntaImagem")
        );
